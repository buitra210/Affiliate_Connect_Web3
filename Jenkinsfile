pipeline {
    ...
    environment {
        ...
        ORGANIZATION_NAME = "centic"

        // Name of repository, application, etc..
        REPOSITORY_NAME = "console"

        // Staging server information
        STAGING_VM_IP = "167.71.196.226"
        STAGING_URL = "https://staging.centic.io"
        STAGING_DOCKER_PORT = 80
        STAGING_DOCKER_BIND_PORT = 3206

        // Production server information
        PRODUCTION_VM_IP = "167.71.196.226"
        PRODUCTION_URL = "https://centic.io"
        PRODUCTION_DOCKER_PORT = 80
        PRODUCTION_DOCKER_BIND_PORT = 3205
            ...
            stages {
                stage("Build staging image") {
                    steps {
                        ...
                        withCredentials([string(credentialsId: "discord-trava-finance", variable: "WEBHOOK_URL")]) {
                            discordSend(
                                description: "\n \
                        ...
                        withCredentials([usernamePassword(credentialsId: 'docker-devopstovchain', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME'),
                                        file(credentialsId: 'env-centic-console-staging', variable: 'ENV_CENTIC_CONSOLE_STAGING')]) {
                            writeFile(file: ".env", text: readFile("${ENV_CENTIC_CONSOLE_STAGING}"), encoding: "UTF-8")
            ...
            stages {
                stage("Build production image") {
                    steps {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'docker-devopstovchain', passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME'),
                                         file(credentialsId: 'env-centic-console-production', variable: 'ENV_CENTIC_CONSOLE_PRODUCTION')]) {
                            writeFile(file: ".env", text: readFile("${ENV_CENTIC_CONSOLE_PRODUCTION}"), encoding: "UTF-8")
                            sh "docker build -t ${env.IMAGE_NAME} . && \
                                docker login -u ${DOCKER_USERNAME} -p ${DOCKER_PASSWORD} ${REGISTRY} && \
                                docker push ${env.IMAGE_NAME} && \
                                docker logout"
