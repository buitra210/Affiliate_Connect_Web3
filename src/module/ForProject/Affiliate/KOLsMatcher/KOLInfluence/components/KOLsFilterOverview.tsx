...
import { toggleKOLsOverviewFilter } from "@centic-scoring/redux/slices/kols-report";
import React, { useState } from "react";

export default function KOLsFilterOverview() {
  ...
  const id = open ? "filter-menu-kol-oveview" : undefined;
  return (
    <Box>
      <Box
        sx={{
      ...
      >
        <Typography fontWeight={500} sx={{ display: "flex", alignItems: "center" }}>
          KOLs/Influencers ({Object.values(filter.kolsOverview || {}).filter((i) => i).length})
...
const Content = () => {
  ...
  return (
    <Paper sx={{ py: 3 }}>
      <Box sx={{ pl: 1.5, pr: 3 }}>
        {Object.entries(filter.kolsOverview).map(([key, data]) => {
          return (
            <Box
              key={key}
              sx={{ display: "flex", cursor: "pointer" }}
              onClick={() => {
                dispatch(toggleKOLsOverviewFilter(key));
              }}
            >
