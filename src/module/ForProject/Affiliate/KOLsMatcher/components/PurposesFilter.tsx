...
export default function PurposesFilter() {
...
function Content({ handleClose }: { handleClose: () => void }) {
  const { purposes, kolsFilter } = useKOLsSelector();
  const dispatch = useAppDispatch();

  const [value, setValue] = useState<{ [key: string]: boolean }>(
    Object.fromEntries(
      (kolsFilter.purposes || []).map((i) => {
        return [i, true];
      })
    )
  );
    ...
    >
      <Box>
        ...
        <Box sx={{ maxHeight: "400px", overflowY: "auto", ml: -1.5 }} className="custom-scrollbar">
          {Object.entries(purposes.data || {}).map((i, index) => {
            return (
