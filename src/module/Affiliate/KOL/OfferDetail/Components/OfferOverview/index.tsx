import { RTKOLsBaseInfo } from "@centic-scoring/api/services/affiliate/affiliate";
...
export default function OfferOverview({ data }: { data: RTKOLsBaseInfo }) {
  return (
    <Paper sx={{ backgroundColor: "background.default", p: 4 }}>
      <Box>
        <Box sx={{ display: "flex", alignItems: "center" }}>
          <Avatar src={data?.introduction?.logo} sx={{ width: "38px", height: "38px", mr: 2 }} />
          <Typography variant="h2" fontWeight={600}>
            {data?.introduction?.featureTitle}
          </Typography>
        </Box>
        <TextEditor
          onValueChange={() => {}}
          updateValue={data?.introduction?.description}
          inputType="markdown"
          viewMode
        />
      </Box>
      <Box>
        ...
        <Box>
          <TextEditor
            onValueChange={() => {}}
            updateValue={data?.requirements?.custom}
            viewMode
            inputType="markdown"
          />
          <Box
          ...
          >
            ...
            >
              {data?.requirements?.kpis.map((kpi, index) => {
                return (
                  <KPIDetailRow
                    key={index}
                    type={kpi.goal}
      ...
      <Box mt={5}>
        ...
        <Box>
          {data?.payments?.type === "Selfpay" && (
            <TextEditor
              onValueChange={() => {}}
              updateValue={data?.payments.custom}
              inputType="markdown"
              viewMode
            />
          )}
            ...
            >
              ...
              >
                ...
                <Typography fontWeight={500} color="text.primary">
                  {`${data?.payments?.payInfo.amount} ${data?.payments?.payInfo.token.name}`}
                </Typography>
              </Box>
              <Box
                sx={{
              ...
              >
                ...
                <Typography fontWeight={500} color="text.primary">
                  {chainsConfig[data?.payments?.payInfo.chainId || ""]?.name}
                </Typography>
              </Box>
            </Box>
            {data?.payments?.bonusReward && (
              <>
                ...
                >
                  {data?.payments?.bonusReward?.map((v, index) => {
                    return (
                      <Box
                        key={index}
                        sx={{
      ...
      <Box mt={5}>
        ...
        <TextEditor
          onValueChange={() => {}}
          updateValue={data?.contactInformation}
          inputType="markdown"
          viewMode
        />
      </Box>
